...
t_double3			raytracer(t_vector ray, t_scene *scene, t_object *to_ignore, int depth)
{
	...
	else
	{
		...
		if (surface->object->reflex > 0.01)
		{
			...
			color_hit = v_plus_v(scale_v(color_hit, 1 - surface->object->reflex),
			scale_v(reflected_color, surface->object->reflex));
		}
		...
		if (surface->object->transparency > 0.01)
		{
			...
			refracted_ray = refract(ray.dir, surface->normal, surface->object->refraction);
...
void			render(t_env *env)
{
	...
	while (++y < HEIGHT)
	{
		...
		while (++x < WIDTH)
		{
			...
			pixel_camera = rotation(pixel_camera, env->scene->camera.dir, REGULAR_MATRIX);
			color = raytracer((t_vector){env->scene->camera.pos, pixel_camera}, env->scene, NULL, 0);
