...
/*   Updated: 2017/05/11 20:05:02 by malorin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_double2 distance, t_scene *scene)
{
	...
	tmp->distance = -1;
	if (distance.x > 0 && is_between_cuts(ray_s, distance.x, object) &&
		!is_in_negative(scene, ray_r, distance.x))
	{
		tmp->distance = distance.x;
		tmp->normal = get_normal(object,
			find_point(ray_s.pos, ray_s.dir, tmp->distance));
	}
	if (distance.y > 0 && is_between_cuts(ray_s, distance.y, object) &&
		!is_in_negative(scene, ray_r, distance.y))
	{
