...
t_double3			planar_mapping(t_surface *surface, t_vector ray, t_object *object)
{
	...
	{
		u = fmod(abs_double(point.x) ,(object->texture->width / 100.0)) / (object->texture->width / 100.0);
		v = fmod(abs_double(point.y) ,(object->texture->height / 100.0)) / (object->texture->height / 100.0);
	}
	return(get_color_from_texture(object->texture, u, v));
}
...
t_double3			cylindrical_mapping(t_surface *surface, t_vector ray, t_object *object)
{
	...
	if (object->dcp)
		...
		v = fmod(abs_double(vp.z) ,(object->texture->height / 100.0)) / (object->texture->height / 100.0);
	if (dot_product(cross_product(vn, ve), vp) > 0)
		u = theta;
	else
		u = 1 - theta;
