...
static void		draw_object_menu(t_env *env)
{
	mlx_string_put(env->mlx, env->win_menu, 325, 115, 0xF00D532, "Liste des objets");
	if (env->menu->index == 0)
		mlx_string_put(env->mlx, env->win_menu, 130, 300, 0xF00D532, "->");
	mlx_string_put(env->mlx, env->win_menu, 150, 300, 0xF00D532, "Objects");
	if (env->menu->index == 1)
		mlx_string_put(env->mlx, env->win_menu, 130, 350, 0xF00D532, "->");
	mlx_string_put(env->mlx, env->win_menu, 150, 350, 0xF00D532, "Lights");
}
